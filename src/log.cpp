...
void logMOEAD(Population &individual_list, std::vector<Fitness> &fitness_list)
{
    for (std::size_t i = 0; i < individual_list.size(); ++i)
    {
        // log_file << individual_list[i].binary_gene << '\n';
        // log_file << individual_list[i].permutation_gene << '\n';
        log_file << fitness_list[i] << '\n';
        log_file << "I\n";
    }
    log_file << "P\n";
    log_file.flush();
