...
Individual repair(const Individual &individual, const Problem &problem)
{
    ...
    {
        if (individual.binary_gene[first_drone_customer_index] == 1)
            ++count;
    }
...
Individual create_random_individual(const Problem &problem)
{
    ...
    {
        if (!problem.can_drone_serve(customer))
            individual.permutation_gene[index++] = customer;
    }
    for (int customer = 1; customer <= problem.customer_count(); ++customer)
    {
        if (problem.can_drone_serve(customer))
...
int countOnes(const BinaryGene &chromosome)
{
    ...
    {
        if (abs(gene - 1.0) < 1e-6)
        { // So sánh với sai số cho số thực
            count += 1;
        }
    }
    return count;
...
int tournamentSelection(std::vector<Individual> &population, int tournamentSize = 3)
{
    std::uniform_int_distribution<int> dist(0, population.size() - 1);
    int best = dist(random_engine);

    for (int i = 1; i < tournamentSize; ++i)
    {
        int current = dist(random_engine);
        if (current < best)
        {
            best = current;
        }
...
int rouletteWheelSelection(std::vector<Individual> &population, std::vector<double> &fitness_list)
{
    double totalFitness = 0;
    for (double fitness : fitness_list)
    {
                                        ...
                                        double lamda, int populationSize)
{
    ...
    {
        double f_val = fitness_list[i][0] * lamda + fitness_list[i][1] * (1 - lamda);
        fitnesses.push_back(f_val);
    }
    std::set<int> check;
    // Elitism Selection - 40%
    int elitism_size = int(0.4 * populationSize);
    for (int i = 0; i < elitism_size; ++i)
    {
        result.push_back(allIndividuals[i]);
        check.insert(i);
    }

    // Tour Selection - 30%
    int torunament_size = int(0.3 * populationSize);
    for (int i = 0; i < torunament_size; ++i)
    {
        int idx = tournamentSelection(allIndividuals, 4);
        while (check.count(idx) > 0)
        {
            idx = tournamentSelection(allIndividuals, 4);
        }
        check.insert(idx);
        result.push_back(allIndividuals[idx]);
    }
    // Roulette Wheel Selection - 30%
    int roulette_size = populationSize - result.size();
    for (int i = 0; i < roulette_size; ++i)
    {
        int idx = rouletteWheelSelection(allIndividuals, fitnesses);
        while (check.count(idx) > 0)
        {
            idx = rouletteWheelSelection(allIndividuals, fitnesses);
        }
        check.insert(idx);
        result.push_back(allIndividuals[idx]);
    }
